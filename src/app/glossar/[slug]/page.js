...
export default function GlossarTerm() {
  ...
  const related = term ? glossarTerms.filter(t => term.relatedSlugs.includes(t.slug)) : []
  ...
  const definedTermSchema = term ? {
    ...
    inDefinedTermSet: {
      '@type': 'DefinedTermSet',
      name: 'Glossar Baufinanzierung – ZR Baufinanzierung',
      url: 'https://www.zr-baufi.de/glossar',
    },
    url: `https://www.zr-baufi.de/glossar/${term?.slug}`,
  } : null

  if (!term) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--hell)' }}>
        <div style={{ textAlign: 'center' }}>
          <h1 style={{ fontFamily: 'var(--font-cormorant)', fontSize: '2rem', color: 'var(--anthrazit)', marginBottom: '1rem' }}>Begriff nicht gefunden</h1>
          <a href="/glossar" style={{ color: 'var(--lila-dark)', textDecoration: 'none', borderBottom: '1px solid var(--lila-dark)', paddingBottom: '2px' }}>← Zurück zum Glossar</a>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .related-card { background: white; border: 1px solid var(--lila-light); padding: 1rem 1.25rem; text-decoration: none; display: block; transition: border-color 0.2s, box-shadow 0.2s; }
        .related-card:hover { border-color: var(--lila); box-shadow: 0 4px 16px rgba(187,159,213,0.2); }
        ...
        @media (max-width: 768px) {
          ...
          .term-content { padding: 3rem 5% !important; }
        }
      `}</style>
      ...
      <section style={{
        ...
        <div style={{ padding: '1.5rem 8% 0', position: 'relative', zIndex: 1 }}>
          <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.4)' }}>
            <a href="/" style={{ color: 'rgba(255,255,255,0.4)', textDecoration: 'none' }}>Start</a>
            <span style={{ margin: '0 0.5rem' }}>›</span>
            <a href="/glossar" style={{ color: 'rgba(255,255,255,0.4)', textDecoration: 'none' }}>Glossar</a>
            <span style={{ margin: '0 0.5rem' }}>›</span>
            <span style={{ color: 'rgba(255,255,255,0.65)' }}>{term.term}</span>
          </p>
        </div>
        <div style={{ padding: '3rem 8% 4rem', position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <p style={{ fontSize: '0.72rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--lila)', marginBottom: '0.75rem' }}>{term.category}</p>
          <h1 style={{ fontFamily: 'var(--font-cormorant)', fontWeight: 600, color: 'white', fontSize: 'clamp(2.4rem, 4vw, 4rem)', lineHeight: 1.1, marginBottom: '1rem', maxWidth: 700, margin: '0 auto 1rem' }}>
            {term.term}
          </h1>
          <p style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.7, maxWidth: 600, fontWeight: 300, margin: '0 auto' }}>
            {term.shortDef}
          </p>
        </div>
      </section>
      ...
      {/* DEFINITION */}
      <section className="term-content" style={{ background: 'var(--hell)', padding: '5rem 8%' }}>
        <div style={{ maxWidth: 740, margin: '0 auto' }}>
          {term.definition.split('\n\n').map((paragraph, i) => (
            <p key={i} style={{
              fontSize: '1rem', color: 'var(--anthrazit)', lineHeight: 1.9,
          ...
          {term.relatedPages && term.relatedPages.length > 0 && (
            <div style={{ marginTop: '2.5rem', padding: '1.5rem 2rem', background: 'white', border: '1px solid var(--lila-light)', borderLeft: '3px solid var(--lila)' }}>
              <p style={{ fontSize: '0.75rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--lila-dark)', marginBottom: '0.75rem' }}>Weiterführende Seiten</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
                {term.relatedPages.map(page => (
                  <a key={page.href} href={page.href} style={{ color: 'var(--lila-dark)', textDecoration: 'none', fontSize: '0.9rem', fontWeight: 500, borderBottom: '1px solid var(--lila-dark)', paddingBottom: '1px' }}>
                    {page.label} →
                  </a>
                ))}
              </div>
      ...
      {/* RELATED TERMS */}
      {related.length > 0 && (
        <section style={{ background: 'white', padding: '4rem 8%' }}>
          <p style={{ fontSize: '0.75rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--lila-dark)', marginBottom: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem' }}>
          ...
          <div className="related-grid">
            {related.map(r => (
              <a key={r.slug} href={`/glossar/${r.slug}`} className="related-card">
                <div style={{ fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--lila-dark)', marginBottom: '0.35rem' }}>{r.category}</div>
                <div style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--anthrazit)', marginBottom: '0.4rem', fontFamily: 'var(--font-cormorant)' }}>{r.term}</div>
                <div style={{ fontSize: '0.82rem', color: 'var(--text-light)', lineHeight: 1.6, fontWeight: 300 }}>{r.shortDef}</div>
              </a>
            ))}
          </div>
          <div style={{ textAlign: 'center', marginTop: '2rem' }}>
            <a href="/glossar" style={{ fontSize: '0.88rem', color: 'var(--lila-dark)', textDecoration: 'none', borderBottom: '1px solid var(--lila-dark)', paddingBottom: '2px' }}>
      ...
      <section style={{ background: 'var(--anthrazit)', padding: '5rem 8%', textAlign: 'center' }}>
        <h2 style={{ fontFamily: 'var(--font-cormorant)', fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 600, color: 'white', lineHeight: 1.2, maxWidth: 520, margin: '0 auto 1.25rem' }}>
          Was bedeutet das für<br /><em style={{ color: 'var(--lila)', fontStyle: 'italic' }}>Ihre Situation?</em>
        </h2>
        <p style={{ fontSize: '1rem', color: 'rgba(255,255,255,0.55)', lineHeight: 1.8, maxWidth: 420, margin: '0 auto 2.5rem', fontWeight: 300 }}>
          Ich erkläre Ihnen gerne, was {term.term} konkret für Ihre Finanzierung bedeutet – kostenlos und unverbindlich.
        </p>
